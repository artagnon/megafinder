...
import {provideEventService} from '../lib/advanced-filter-file';
import {
...
describe('Functional tests', () => {
    ...
    beforeEach(() => {
        ...
        activationPromise = atom.packages.activatePackage('advanced-filter-file');
    });

    function getUI() {
        return workspaceElement.querySelector('.advanced-filter-file');
    }
    ...
    function openModal() {
        atom.commands.dispatch(workspaceElement, 'advanced-filter-file:toggle');
        waitsForPromise(() => {
    ...
    function resetConfig() {
        atom.config.unset('advanced-filter-file.createFileInstantly');
        atom.config.unset('advanced-filter-file.helmDirSwitch');
        atom.config.unset('advanced-filter-file.defaultInputValue');
    }
    ...
    describe('Path input', () => {
        ...
        function assertAutocompletesTo(inputPath, autocompletedPath) {
            setPath(inputPath);
            dispatch('advanced-filter-file:autocomplete');
        ...
        it('can remove the current path component', () => {
            ...
            dispatch('advanced-filter-file:delete-path-component');
        ...
        it('can switch to the user\'s home directory using a shortcut', () => {
            atom.config.set('advanced-filter-file.helmDirSwitch', true);
    ...
    describe('Path input default value', () => {
        ...
        it('can be configured to be the current file\'s directory', () => {
            atom.config.set(
                'advanced-filter-file.defaultInputValue',
                DEFAULT_ACTIVE_FILE_DIR
            );
            waitsForPromise(() => {
        ...
        it('can be configured to be blank', () => {
            atom.config.set('advanced-filter-file.defaultInputValue', DEFAULT_EMPTY);
            openModal();
    ...
    describe('Undo', () => {
        ...
        it('can undo tab completion', () => {
            ...
            dispatch('advanced-filter-file:autocomplete');
            expect(currentPath()).toEqual(fixturePath('examples') + stdPath.sep);
            dispatch('advanced-filter-file:undo');
    ...
    describe('Opening files', () => {
        ...
        it(`creates the directory when opening a path ending a separator if
            ...
            atom.config.set('advanced-filter-file.createDirectories', true);
            setPath(path);
        ...
        it('creates a new file when configured to', () => {
            ...
            atom.config.set('advanced-filter-file.createFileInstantly', true);
            setPath(path);
            expect(fileExists(path)).toEqual(false);
        ...
        */
        ...
        function moveDown(times) {
            for (let k = 0; k < times; k++) {
                dispatch('advanced-filter-file:move-cursor-down');
            }
        }

        function moveUp(times) {
            for (let k = 0; k < times; k++) {
                dispatch('advanced-filter-file:move-cursor-up');
            }
        }
        ...
        it(`can select the first item in the list if none are selected using
            special command`, () => {
            setPath(fixturePath('prefix'));
            dispatch('advanced-filter-file:confirm-selected-or-first');
