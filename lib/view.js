...
export default class AdvancedFilterFileView {
    constructor() {
        ...
        this.pathEditor.setPlaceholderText('find-pattern.*');
    ...
    get content() {
        return dom(`
            <div class="advanced-filter-file">
    ...
    createModalPanel() {
        ...
        let outsideClickHandler = (ev) => {
            if (ev.target::closest('.advanced-filter-file') === null) {
    ...
    setPath(path) {
        ...
        // Clear path list.
        this.parentDirectoryListItem.classList.remove('selected');
        this.forEachListItem('.list-item:not(.parent-directory)', (listItem) => {
            listItem.remove();
        });

        let paths = path.matchingPaths();
        if (paths.length === 0 || path.fragment !== '') {
