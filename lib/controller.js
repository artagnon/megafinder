...
import MegafindFilterFileView from './view';
...
export class MegafindFilterFileController {
    constructor() {
        this.view = new MegafindFilterFileView();
        this.panel = null;
    ...
    openPath(path) {
        if (path.exists()) {
            ...
            if (atom.config.get('megafinder.createFileInstantly')) {
                path.createFile();
            ...
            emitter.emit('did-open-path', path.absolute);
            this.detach();
        } else if (atom.config.get('megafinder.createDirectories')) {
            path.createDirectories();
