...
import AdvancedFilterFileView from './view';
import {Path} from './models';
import {getProjectPath} from './utils';


// Emitter for outside packages to subscribe to. Subscription functions
// are exponsed in ./advanced-filter-file
export let emitter = new Emitter();


export class AdvancedFilterFileController {
    constructor() {
        this.view = new AdvancedFilterFileView();
        this.panel = null;
        ...
        atom.commands.add('atom-workspace', {
            'advanced-filter-file:toggle': ::this.toggle
        });
        atom.commands.add('.advanced-filter-file', {
            'core:confirm': ::this.confirm,
            'core:cancel': ::this.detach,
            'application:add-project-folder': ::this.addSelectedProjectFolder,
            'advanced-filter-file:autocomplete': ::this.autocomplete,
            'advanced-filter-file:undo': ::this.undo,
            'advanced-filter-file:move-cursor-down': ::this.moveCursorDown,
            'advanced-filter-file:move-cursor-up': ::this.moveCursorUp,
            'advanced-filter-file:confirm-selected-or-first': ::this.confirmSelectedOrFirst,
            'advanced-filter-file:delete-path-component': ::this.deletePathComponent,
        });
    ...
    pathChange(newPath)  {
        ...
        // shortcuts.
        if (atom.config.get('advanced-filter-file.helmDirSwitch')) {
            let sep = newPath.sep;
    ...
    openPath(path) {
        if (path.exists()) {
            ...
            if (atom.config.get('advanced-filter-file.createFileInstantly')) {
                path.createFile();
            ...
            emitter.emit('did-open-path', path.absolute);
            this.detach();
        } else if (atom.config.get('advanced-filter-file.createDirectories')) {
            path.createDirectories();
