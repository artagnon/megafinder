...
export class Finder {
    constructor() {
        ...
        let sep = preferredSeparatorFor(projectPath);
        let dirNarrowing = [projectPath];
        let configStr = 'megafinder.patternNarrowing';
        let patternNarrowing = atom.config.get(configStr).split(' ');
        let mwAnchor = new Path(projectPath + sep + 'mw_anchor');
        if (mwAnchor.exists()) {
            configStr = 'megafinder.MWspecifics.dirNarrowing';
            dirNarrowing = atom.config.get(configStr).split(' ');
            dirNarrowing = dirNarrowing.map((name) => projectPath + sep + name);
            configStr = 'megafinder.MWspecifics.patternNarrowing';
            patternNarrowing = atom.config.get(configStr).split(' ');
        }
        let stdoutOutput = '';
        let stderrOutput = '';
        patternNarrowing0 = ['-name', patternNarrowing[0]];
        patternNarrowing.slice(1).map(
                pat => patternNarrowing0 =
                  patternNarrowing0.concat(['-or', '-name', pat]));

        defineImmutable(this, 'projectPath', projectPath);
        defineImmutable(this, 'patternNarrowing', patternNarrowing0);
        defineImmutable(this, 'dirNarrowing', dirNarrowing);
        defineMutable(this, 'stdoutOutput', stdoutOutput);
        defineMutable(this, 'stderrOutput', stderrOutput);
    }
    filenames() {
        return new Promise((resolve, reject) => {
            ...
            let parameters = {
              command: 'find',
              args: this.dirNarrowing
                .concat(['-type', 'f'])
                .concat(this.patternNarrowing),
              ...
              exit: function(status) {
                if (status == 1) {
                    ...
                    atom.notifications.addInfo(`stdErr: ${this.stderrOutput}`)
                    reject(this.stdoutOutput);
                } else {
                    stdoutOutput = stdoutOutput.match(/[^\r\n]+/g);
