...
export let fuzzyMatcher = {
    match(pattern, filenames) {
        if (pattern) {
            filenames = filenames.map(n => n.slice(getProjectPath().length));
            filenames = fuzzaldrinPlus.filter(filenames, pattern);
...
export class Path {
    ...
    directoryFilenames() {
        return new Promise((resolve, reject) => {
            ...
            let dirNarrowing = [dirName];
            let battreePath = new Path(dirName + this.sep + 'battree');
            if (battreePath.exists()) {
                configStr = 'advanced-filter-file.defaultMWdirNarrowing';
                dirNarrowing = atom.config.get(configStr).split(' ');
                dirNarrowing = dirNarrowing.map((name) => dirName + this.sep + name);
            }
            let stdoutOutput = "";
            let stderrOutput = "";
            if (dirName == null) {
