...
import {configuredMatcher} from './matchers';
import {
    absolutify,
    cachedProperty,
    defineImmutable,
...
export class Path {
    ...
    directoryFilenames() {
        return new Promise(function(resolve, reject) {
            ...
            let parameters = {
              command: "find",
              args: [absoluteDir, "-name *.cpp", "-or -name *.hpp",
              "-or -name *.c", "-or -name *.h"],
              ...
              exit: function(status) {
                ...
                atom.notifications.addInfo(`Output: ${stdOutput}`);
                resolve(stdoutOutput.split("\n"));
              }
            };
            const spawnedProcess = new atom.BufferedProcess(parameters);
            reject(stderrOutput);
        });
    }

    matchingPaths() {
        return configuredMatcher().match(this, this.directoryFilenames());
    }
